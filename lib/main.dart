...
class _SplashScreenState extends State<SplashScreen> {
  ...
  /// Uses AssetManifest.json to copy every file under assets/www.
  Future<String> _copyWebAssets() async {
    ...
    final filesToCopy = manifest.keys
        .where((key) => key.startsWith('assets/www/'))
        .toList()
      ..sort();

    if (filesToCopy.isEmpty) {
      debugPrint('No assets found in AssetManifest for assets/www/. Falling back to hardcoded list.');
    }
    ...
    for (final assetPath in filesToCopy) {
      try {
        ...
        debugPrint('✓ Copied: $assetPath -> ${targetFile.path} ($fileSize bytes)');
        successCount++;
      } catch (e) {
...
class _WebViewScreenState extends State<WebViewScreen> {
  ...
  Future<void> _startServer() async {
    try {
      ...
      // assets (flutter_assets/...). The web assets are included in the
      // Flutter asset bundle under: assets/www/
